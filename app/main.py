...
origins = [
    "http://localhost",
    "http://127.0.0.1",
    "http://localhost:5500",  # Αν χρησιμοποιείς Live Server
    "http://127.0.0.1:5500"
]
...
app.add_middleware(
    ...
    allow_origins=origins,        # ποιοι clients επιτρέπονται
    allow_credentials=True,
    allow_methods=["*"],          # GET, POST, PUT, DELETE, OPTIONS κτλ
    allow_headers=["*"],
)
...
def get_my_projects(
    ...
    # Aliases για να κάνουμε join δύο φορές τον ίδιο πίνακα (files)
    File1 = aliased(File)
    File2 = aliased(File)
    ...
    # Μετατρέπουμε τα αποτελέσματα σε JSON-friendly
    return [
        {
            "id": p.id,
            "name": p.name,
